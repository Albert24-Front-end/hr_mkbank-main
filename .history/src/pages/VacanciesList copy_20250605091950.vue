...
const rules = computed(() => ({
  searchField: {
    minLength: helpers.withMessage(() => t('error_message'), minLength(5)),
  },
}))
...
const filteredVacancies = ref(vacancies.value)
...
function handleSearchBtnClick() {
  ...
  filteredVacancies.value =
    vacancies.value.filter((v) => {
      ...
      return locationMatch && directionMatch && (searchField.value.length >= 5 ? titleMatch : [])
    }) ?? []
}
...
const showSearchFieldError = computed(() =>
  v.value.searchField.$dirty && v.value.searchField.$errors.length > 0
)
</script>
      ...
      <div class="mt-10 grid grid-cols-1 lg:grid-cols-6 gap-6">
          ...
          <div class="w-full px-5 py-5 space-y-4 bg-white shadow-lg rounded-md">
            <div class="flex justify-between items-center gap-10">
              ...
              <button
                type="button"
                @click="removeFilters"
                class="text-red-500 hover:cursor-pointer hover:underline hover:underline-offset-2"
                id="remove"
              >
                {{ $t('filter.clear') }}
              </button>
            ...
            <div class="py-5 flex flex-col gap-4 text-sm">
              <div class="flex flex-col gap-2">
                ...
                <select
                  ...
                  class="w-full rounded-md outline-solid outline-1 px-2 py-1 bg-default text-base text-gray-500 disabled:cursor-not-allowed disabled:opacity-75 focus:outline-none focus:ring-2 focus:ring-primary focus:border-sky-500 focus:shadow-md transition duration-200 ease-in-out"
                >
        ...
        <div class="lg:col-span-4 w-full space-y-4">
          <form class="flex items-center gap-3" @submit.prevent="handleSearchBtnClick">
            ...
            <button
              type="submit"
              class="py-3 px-6 text-sm text-white bg-primary rounded-lg hover:cursor-pointer hover:bg-primary/50"
              @click="handleSearchBtnClick"
            >
              {{ $t('vacancies-list.search_btn') }}
            </button>
          </form>
          <div v-if="showSearchFieldError" class="text-red-500 text-sm mt-1">
            <p v-for="e in v.searchField.$errors" :key="e.$uid">{{ e.$message }}</p>
          </div>
          <VacancyCard
