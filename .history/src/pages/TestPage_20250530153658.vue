...
const queryItems = computed(()=>{
  ...
  let search = queryText.value
  // if(search) {
  //   return p = p.filter((fruit)=> fruit.title.toLowerCase().indexOf(search.toLowerCase()) !== -1)
  // }
  //   return
  return p ? p.filter(f=> {
    const titleMatch = !search || f.title.toLowerCase().includes(search)
    return titleMatch
  }) : []
})

function handleQuery(){
  queryText.value = query.value.trim().toLowerCase()
}
...
<template>
  <section class="w-full py-20 bg-gray-50" id="vacancies-list">
    <div class="@container mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <form class="flex items-center gap-3 mb-10" @submit.prevent="handleQuery">
        <input
          type="text"
