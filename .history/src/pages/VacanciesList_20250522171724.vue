...
const { directions: departments, locations } = useVacanciesListFilterData()
          ...
          <form class="w-full px-5 py-5 space-y-4 bg-white shadow-lg rounded-md">
            ...
            <div class="py-5 flex flex-col gap-4 text-sm">
              <div class="flex flex-col gap-2 ">
                <label for="department-select">{{ t('filter.labels.direction') }}</label>
                <select name="departments" id="department-select" class="w-full rounded-md outline-solid outline-1 px-2 py-1 bg-default text-base text-gray-500 disabled:cursor-not-allowed disabled:opacity-75 focus:outline-none focus:ring-2 focus:ring-primary focus:border-sky-500 focus:shadow-md transition duration-200 ease-in-out">
                  ...
                  <option v-for="d in departments" :key="d.name" :value="d.name">{{ d.text() }}</option>
                </select>
              </div>
              <div class="flex flex-col gap-2">
                ...
                <select name="location" id="location-select" class="w-full rounded-md outline-solid outline-1 px-2 py-1 bg-default text-base text-gray-500 disabled:cursor-not-allowed disabled:opacity-75 focus:outline-none focus:ring-2 focus:ring-primary focus:border-sky-500 focus:shadow-md transition duration-200 ease-in-out">
                  ...
                  <option v-for="l in locations" :key="l.name" :value="l.name">{{ d.text() }}</option>
                </select>
              </div>
            </div>
