...
const dirtyField = computed(()=>{
  return v.searchField.$dirty && v.searchField.$errors.length > 0
})
...
<template>
  <section class="w-full py-20 bg-gray-50" id="vacancies-list">
    <div class="@container mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="dirtyField" class="text-red-500 text-sm mt-1">
        <p v-for="e in v.searchField.$errors" :key="e.$uid">{{ e.$message || 'Invalid value' }}</p>
      </div>
      <!-- <TransitionGroup>
